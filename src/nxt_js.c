...
nxt_js_add_tpl(nxt_js_conf_t *jcf, nxt_str_t *str, nxt_bool_t strz)
{
    ...
    static nxt_str_t  func_str = nxt_string("function(method, uri, version, "
                                            "host, remoteAddr, status, args, "
                                            "headers) {"
                                            "    return ");
    static nxt_str_t  strz_str = nxt_string(" + '\0'");
...
nxt_js_compile(nxt_js_conf_t *jcf)
{
    ...
    *p++ = ']';
    ...
    nxt_js_proto_id = njs_vm_external_prototype(jcf->vm, jcf->proto,
                                                jcf->protos);
    if (nxt_js_proto_id < 0) {
        return NXT_ERROR;
    }

    ret = njs_vm_compile(jcf->vm, &start, p);

    return (ret != NJS_OK) ? NXT_ERROR : NXT_OK;
}
...
nxt_int_t
nxt_js_call(nxt_js_cache_t *cache, nxt_js_t *js, nxt_str_t *str, void *ctx)
{
    njs_vm_t            *vm;
    njs_int_t           ret;
    njs_str_t           res;
    ...
    njs_function_t      *func;
    njs_opaque_value_t  opaque_value, arguments[8];

    static const njs_str_t  method_str = njs_str("method");
    static const njs_str_t  uri_str = njs_str("uri");
    static const njs_str_t  version_str = njs_str("version");
    static const njs_str_t  host_str = njs_str("host");
    static const njs_str_t  remote_addr_str = njs_str("remoteAddr");
    static const njs_str_t  status_str = njs_str("status");
    ...
    (void) njs_vm_object_prop(vm, njs_value_arg(&opaque_value), &method_str,
                              &arguments[0]);
    (void) njs_vm_object_prop(vm, njs_value_arg(&opaque_value), &uri_str,
                              &arguments[1]);
    (void) njs_vm_object_prop(vm, njs_value_arg(&opaque_value), &version_str,
                              &arguments[2]);
    (void) njs_vm_object_prop(vm, njs_value_arg(&opaque_value), &host_str,
                              &arguments[3]);
    (void) njs_vm_object_prop(vm, njs_value_arg(&opaque_value),
                              &remote_addr_str, &arguments[4]);
    (void) njs_vm_object_prop(vm, njs_value_arg(&opaque_value), &status_str,
                              &arguments[5]);
    (void) njs_vm_object_prop(vm, njs_value_arg(&opaque_value), &args_str,
                              &arguments[6]);
    (void) njs_vm_object_prop(vm, njs_value_arg(&opaque_value), &headers_str,
                              &arguments[7]);

    ret = njs_vm_call(vm, func, njs_value_arg(&arguments), 8);

    /* TODO check return */
    (void) njs_vm_retval_string(vm, &res);

    if (ret != NJS_OK) {
        nxt_alert(&nxt_main_task, "js exception: %V", &res);
        return NXT_ERROR;
    }
